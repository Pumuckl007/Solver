...
["", "src/matrix.dart", , O, {
  "^": "",
  Matrix: {
    "^": "Object;matrix,matrixValues,xSize,ySize,undefined",
    getrow$1: function(rownumber) {
      var t1, row, t2, t3, i, t4;
      if (!this.undefined) {
        t1 = this.xSize;
        row = Array(t1);
        row.$builtinTypeInfo = [P.$double];
        for (t2 = this.xSize, t3 = this.matrix, i = 0; i < t2; ++i) {
          if (rownumber >= t3.length)
            return H.ioore(t3, rownumber);
          t4 = t3[rownumber];
          if (i >= t4.length)
            return H.ioore(t4, i);
          t4 = t4[i];
          if (i >= t1)
            return H.ioore(row, i);
          row[i] = t4;
        }
        return row;
      }
    ...
    getcolumn$1: function(columnnumber) {
      var row, t1, i, line, t2;
      if (!this.undefined) {
        row = H.setRuntimeTypeInfo(Array(this.ySize), [P.$double]);
        for (t1 = row.length, i = 0; i < this.ySize; ++i) {
          line = C.JSInt_methods.toString$0(i) + " " + C.JSInt_methods.toString$0(columnnumber);
          H.printString(line);
          t2 = this.matrix;
          if (columnnumber >= t2.length)
            return H.ioore(t2, columnnumber);
          t2 = t2[columnnumber];
          if (i >= t2.length)
            return H.ioore(t2, i);
          t2 = t2[i];
    ...
    multiply$1: function(m) {
      var t1, valuestore, k, t2, i, row, column, value, z, t3, t4;
      t1 = this.xSize;
      if (t1 === m.ySize && !this.undefined && !m.undefined) {
        valuestore = H.setRuntimeTypeInfo(Array(m.xSize * t1), [P.$double]);
        for (t1 = valuestore.length, k = 0; t2 = m.xSize, k < t2; ++k)
          for (i = 0; i < this.xSize; ++i) {
            row = this.getrow$1(k);
            column = m.getcolumn$1(i);
            for (t2 = this.xSize, value = 0, z = 0; z < t2; ++z) {
              if (z >= row.length)
                return H.ioore(row, z);
              t3 = row[z];
              if (z >= column.length)
                return H.ioore(column, z);
              t4 = column[z];
              if (typeof t3 !== "number")
                return t3.$mul();
              if (typeof t4 !== "number")
            ...
            t2 = k * m.xSize + i;
            if (t2 >= t1)
              return H.ioore(valuestore, t2);
            valuestore[t2] = value;
          }
        return O.Matrix$(valuestore, t2, this.xSize);
      }
      t1 = new O.Matrix(null, null, 0, 0, false);
      t1.undefined = true;
      return t1;
    },
    toString$0: function(_) {
      var string, r, c, t1, t2;
      for (string = "", r = 0; r < this.xSize; ++r)
        for (c = 0; c < this.ySize; ++c) {
          t1 = string + " ";
          t2 = this.matrix;
          if (r >= t2.length)
            return H.ioore(t2, r);
          ...
          if (c >= t2.length)
            return H.ioore(t2, c);
          string = t1 + J.toString$0(t2[c]);
        }
      return string;
    },
    Matrix$3: function(values, x, y) {
      var i, t1, t2, t3, k, t4, t5;
      this.xSize = x;
      this.ySize = y;
      this.matrixValues = values;
      this.matrix = H.setRuntimeTypeInfo(Array(x), [[P.List, P.$double]]);
      for (i = 0; i < x; ++i) {
        t1 = this.matrix;
        t2 = Array(y);
        ...
        if (i >= t1.length)
          return H.ioore(t1, i);
        t1[i] = t2;
        for (t1 = this.matrix, t2 = 3 * i, t3 = values.length, k = 0; k < y; ++k) {
          if (i >= t1.length)
            return H.ioore(t1, i);
          t4 = t1[i];
          t5 = t2 + k;
          if (t5 >= t3)
            return H.ioore(values, t5);
          t5 = values[t5];
          if (k >= t4.length)
            return H.ioore(t4, k);
          t4[k] = t5;
    ...
    static: {Matrix$: function(values, x, y) {
        var t1 = new O.Matrix(null, null, 0, 0, false);
        t1.Matrix$3(values, x, y);
        return t1;
      }}
  }
}],
["", "solver2.dart", , D, {
  ...
  main: [function() {
    ...
    t1 = document.querySelector("#Solve");
    t2 = J.get$onClick$x(t1);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(D.solveIt$closure()), t2._useCapture), [H.getTypeArgumentByIndex(t2, 0)])._tryResume$0();
    $.solve = t1;
  }, "call$0", "main$closure", 0, 0, 1],
  solveIt: [function($event) {
    var coeficents, equals, xyz, answer, t1, t2;
    coeficents = O.Matrix$([1, 2, 1, 7, 5, 8, 3, 5, 7], 3, 3);
    equals = O.Matrix$([2, 2, 2], 1, 3);
    P.print(coeficents.toString$0(0) + " s");
    P.print(equals.toString$0(0));
    xyz = coeficents.multiply$1(equals);
    P.print(xyz.toString$0(0));
    ...
    if (2 >= t1.length)
      return H.ioore(t1, 2);
    answer.textContent = t2 + J.toString$0(t1[2]);
  }, "call$1", "solveIt$closure", 2, 0, 6]
},
1],
]);
...
//# sourceMappingURL=solver2.dart.js.map
{
  var message =
...
function dart_precompiled($collectedClasses) {
  ...
  function Matrix(matrix, matrixValues, xSize, ySize, undefined) {
    this.matrix = matrix;
